...
router.get("/stats", async (req, res) => {
  try {
    ...
    const [callsMain, callsByType, jobs, todos, queueMain, queueByType, quotations, customers] = await Promise.all([
      ...
      // ── Calls by type (call_type stored in metadata JSON) ─────────────────
      db.query(
    ...
    const ct  = callsByType.rows;
    const j   = jobs.rows[0];
    const td  = todos.rows[0];
    const q   = queueMain.rows[0];
    const qct = queueByType.rows;
    const qt  = quotations.rows[0];
    const cu  = customers.rows[0];

    const byCallType = {};
    ct.forEach(r => { byCallType[r.call_type] = Number(r.cnt); });

    const queueByCallType = {};
    qct.forEach(r => { queueByCallType[r.call_type] = Number(r.cnt); });
