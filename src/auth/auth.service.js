...
async function register({ email, password, name, companyName }) {
  ...
  return db.transaction(async (client) => {
    ...
    const userResult = await client.query(
      `INSERT INTO users (company_id, email, first_name, last_name, password_hash, role, is_active) 
       VALUES ($1, $2, $3, $4, $5, $6, true) 
       RETURNING id, company_id, email, first_name, last_name, role, is_active, created_at`,
      [company.id, email.toLowerCase(), first, last, passwordHash, "admin"]
    );
...
async function login(email, password) {
  // Fetch user with company details (exclude soft-deleted)
  const result = await db.query(
    `SELECT u.id, u.company_id, u.email, u.first_name, u.last_name, u.password_hash, u.role, u.is_active,
...
async function changePassword(userId, currentPassword, newPassword) {
  ...
  const result = await db.query(
    "SELECT id, password_hash FROM users WHERE id = $1 AND is_deleted = FALSE",
    [userId]
  );
...
async function getUserById(userId) {
  const result = await db.query(
    `SELECT u.id, u.company_id, u.email, u.first_name, u.last_name, u.role, u.is_active, u.last_login,
            c.name as company_name
     FROM users u
     JOIN companies c ON u.company_id = c.id
     WHERE u.id = $1 AND u.is_deleted = FALSE`,
...
async function getUserByEmail(email) {
  const result = await db.query(
    `SELECT u.id, u.company_id, u.email, u.first_name, u.last_name, u.role, u.is_active, u.supabase_id,
            c.name as company_name
     FROM users u
     JOIN companies c ON u.company_id = c.id
     WHERE u.email = $1 AND u.is_deleted = FALSE`,
...
async function getUserBySupabaseId(supabaseId) {
  const result = await db.query(
    `SELECT u.id, u.company_id, u.email, u.first_name, u.last_name, u.role, u.is_active,
     ...
     FROM users u
     JOIN companies c ON u.company_id = c.id
     WHERE u.supabase_id = $1 AND u.is_deleted = FALSE`,
    [supabaseId]
  );
...
async function updatePasswordByEmail(email, newPassword) {
  ...
  if (result.rowCount === 0) {
    const err = new Error("User not found or account removed");
    err.status = 404;
    throw err;
  }
