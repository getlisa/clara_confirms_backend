...
const VARIABLE_SEEDS = [
  ...
  { name: "next_service_line",      sort_order: 56, resolved_from: "job-confirmation-context (live at dispatch)", description: "SHORT spoken service-line summary for the opening sentence, e.g. 'Backflow, Alarm Systems and Sprinkler'. Categories only — deliberately excludes the descriptions, whose parenthetical detail ('(1-wet)(1-dry)') reads badly aloud. Use next_appointment_services for the detail." },
  { name: "next_appointment_services", sort_order: 59, resolved_from: "job-confirmation-context (live at dispatch)", description: "Every service on the next visit, one per line, as '<service line> — <description>'. The description carries the real detail (equipment, counts, locations) and the line name is the category it belongs to; both are needed, so they are paired here rather than listed separately. Blank when the visit has no services." },
  { name: "next_arrival_window",    sort_order: 62, resolved_from: "job-confirmation-context (live at dispatch)", description: "Arrival window for the next visit, e.g. 'between 8 AM and 9 AM' — one hour FORWARD from the scheduled start, never before it. Precomputed because a model doing this arithmetic gets hour and noon boundaries wrong. Blank on a DST fall-back night, where both bounds are the same wall-clock time; state the scheduled time alone then." },
  { name: "next_technicians",       sort_order: 61, resolved_from: "job-confirmation-context (live at dispatch)", description: "The FULL crew on the next visit, one per line, as 'Name (phone, email)'. Most multi-service visits send two to four technicians; next_technician is only the short spoken list of their names." },
