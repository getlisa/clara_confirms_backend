...
Every conversation is executed by a voice AI agent (Retell), an SMS/chat agent, or a
link-based web chat widget. Every outcome is written back — into the platform's own
...
- **G2** — Reach the contact on whatever channel actually works: voice, SMS, or web chat link.
...
- **F1.5** Automatic sync every 6 hours via cron, plus an on-demand manual trigger from
  the UI.
...
**Purpose:** a third channel that sidesteps SMS/A2P approval entirely.

**Requirements**

- **F6.1** An opaque, unguessable token resolves to a specific job or appointment's
  conversation context.
- **F6.2** Links are optionally expiring and track `last_opened_at`.
- **F6.3** The public resolve endpoint is token-authed only — no login, no tenant leakage.
- **F6.4** The chat widget runs the same conversation flow and the same tools as voice.
...
**Failure branch:** no answer → retry per `max_attempts`; if the tenant is
`voice_then_sms_fallback`, attempt 2 goes out as SMS. Exhausted attempts → `NOT_PICKED` or
`VOICEMAIL` todo for a human.
...
For a tenant without A2P approval: the dispatcher resolves `web_chat`, generates a
tokenized link scoped to the appointment, and emails it. The customer opens the link and
has the same tool-backed conversation in a browser. State persists if they close the tab.
...
| Retell | Voice and chat agents | Provisioning drift between Clara config and Retell state. Mitigation: prompt sync and per-company re-provisioning |
| Twilio | SMS delivery | A2P 10DLC approval is manual and slow. Mitigation: ops-tracked `sms_status` plus the web-chat channel, which has no A2P dependency |
| SendGrid | Chat link and service link email | Deliverability. Mitigation: per-message status tracking and todos on failure |
| OpenAI / Groq | Copilot reasoning | Cost and latency |
...
- Migrations are additive by convention — columns are added, not renamed or dropped, so a
  deploy can be rolled back without data loss. Contract migrations that drop columns are
  parked and applied only after the code cutover is verified.
...
tools · scheduling, office hours, retries, concurrency · SMS channel · web chat links ·
...
| **Channel** | The medium: voice, SMS, or web chat |
